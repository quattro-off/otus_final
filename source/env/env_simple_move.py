...
class HumanMoveSimpleAction(gym.Env):
    ...
    def calc_step_reward_box(self, set_angle_speed: float, set_speed_forward: float, set_speed_right: float):
        ...
        if vec_to_finish.rho < self.finish_radius:
            ...
            if self.time_model_optimum > self.time_model:
                return 5, True, False
            else:
                time_fine = self.time_model/self.time_model_optimum
                return self._clamp( 5 - time_fine, 1, 5), True, False
        ...
        # стандартный штраф
        self.angle_step_reward = 0.0005 * (0.5 * math.pi - abs(delta_angle))
        ...
        if self.speed_bind_set.rho > 0.5 and vec_to_finish.rho > 5:
            ...
            koef_rew_view_to_speed = cos_view_to_speed - 0.9
            d_right_reward = -0.001 if cos_view_to_speed > 0.9 and abs(self.speed_bind_set.y) > 0.01 else 0.0001
            self.view_step_reward = 0.003 * koef_rew_view_to_speed + d_right_reward
