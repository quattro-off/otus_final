...
class HumanMoveAction(gym.Env):
    ...
    def step(self, action, is_teacher: bool = False):
        ...
        b_move_to_view  = int(1)                    # action - 1
        set_angle_dir   = self.dir_move.phi         # action - 2(-) - 3(+)
        set_angle_view  = self.dir_view.phi         # action - 4(-) - 5(+)
        set_speed       = self.speed                # action - 6(-) - 7(+)
        set_state       = int(0)                    # action - 8
        set_action      = int(0)                    # action - 9
        ...
        if self.continuous == True:
            ...
            if action == 1:
                ...
                set_angle_view = self.dir_view.phi - 0.5
            elif action == 5:
                set_angle_view = self.dir_view.phi + 0.5
            elif action == 6:
        ...
        if self.continuous == True:
            ...
            if abs(delta_angle) > 5 * math.pi / 180 :
                if self.speed > 1:
                    ...
                    t_action = 3 if delta_angle >= 0 else 2
            else:
                if teach_set_dir.rho < 1:
                    t_action = 6 if self.speed > 0 else 7
                elif teach_set_dir.rho < 4:
                    t_action = 6 if self.speed >= 0.5 else 7
                else:
                    t_action = 7
